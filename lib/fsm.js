...
export async function runAgent({ read, parse, categorize, onTransition }) {
  ...
  try {
    ...
    // The [ERROR] state tag already labels this as a failure, so the message
    // itself stays a plain-English sentence (no "Read failed:" prefix).
    emit(STATES.ERROR, err.message, "error");
